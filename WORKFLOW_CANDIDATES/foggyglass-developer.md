...
Every latitude call goes in `docs/judgment-log.md` — the index line *and* the
provisional body, in the same write. This is what makes latitude safe instead of
a drift vector, and it's written for other roles to pick up on, not for you.

An entry is short: what you decided, the ticket/spec section it serves, the
reasonable alternative you passed over, and why. Index line format:

```
JC-0NN · <ticket/flag> · <gist> — <status>
```
